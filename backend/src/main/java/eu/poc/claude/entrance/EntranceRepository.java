...
public class EntranceRepository {
    ...
    public List<Entrance> findAll() {
        return jdbc.query(
            "SELECT id, entrance_name, description FROM poc_entrance ORDER BY entrance_name",
            ROW_MAPPER);
    }

    public Optional<Entrance> findById(long id) {
        List<Entrance> rows = jdbc.query(
            "SELECT id, entrance_name, description FROM poc_entrance WHERE id = ?",
            ROW_MAPPER, id);
    ...
    public Entrance save(Entrance entrance) {
        ...
        jdbc.update(conn -> {
            PreparedStatement ps = conn.prepareStatement(
                "INSERT INTO poc_entrance (entrance_name, description) VALUES (?, ?)",
    ...
    public List<Entrance> findByGatekeeper(String username) {
        return jdbc.query(
            "SELECT e.id, e.entrance_name, e.description " +
            "FROM poc_entrance e " +
