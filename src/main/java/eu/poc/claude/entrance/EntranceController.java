...
public class EntranceController {
    ...
    // ── GET /api/entrances/available  (any authenticated user — for inviter dropdown) ──

    @GetMapping("/available")
    public List<Entrance> available(
            @RequestHeader(value = "Authorization", required = false) String authHeader) {
        requireUsername(authHeader); // authentication only — no role restriction
        return repo.findAll();
    }

    // ── GET /api/entrances  (admin: all entrances) ────────────────────────────
