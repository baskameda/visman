...
var useRtl = () => {
  const value = React10.useContext(RtlContext);
  return value ?? false;
};
...
function useCurrentColorScheme(options) {
  ...
  const modeStorage = React14.useMemo(() => storageManager == null ? void 0 : storageManager({
    key: modeStorageKey,
    storageWindow
  }), [storageManager, modeStorageKey, storageWindow]);
  const lightStorage = React14.useMemo(() => storageManager == null ? void 0 : storageManager({
    key: `${colorSchemeStorageKey}-light`,
    storageWindow
  }), [storageManager, colorSchemeStorageKey, storageWindow]);
  const darkStorage = React14.useMemo(() => storageManager == null ? void 0 : storageManager({
  ...
  const [isClient, setIsClient] = React14.useState(noSsr || !isMultiSchemes);
  React14.useEffect(() => {
    setIsClient(true);
  }, []);
  const colorScheme = getColorScheme(state);
  const setMode = React14.useCallback((mode) => {
  ...
  const setColorScheme = React14.useCallback((value) => {
    if (!value) {
      setState((currentState) => {
  ...
  const handleMediaQuery = React14.useCallback((event) => {
  ...
  const mediaListener = React14.useRef(handleMediaQuery);
...
function createCssVarsProvider(options) {
  ...
  const ColorSchemeContext = React15.createContext(void 0);
  if (true) {
    ColorSchemeContext.displayName = "ColorSchemeContext";
  }
  const useColorScheme = () => React15.useContext(ColorSchemeContext) || defaultContext;
  ...
  function CssVarsProvider(props) {
    ...
    const hasMounted = React15.useRef(false);
    const upperTheme = useTheme3();
    const ctx = React15.useContext(ColorSchemeContext);
    const nested = !!ctx && !disableNestedContext;
    const initialTheme = React15.useMemo(() => {
      if (themeProp) {
        return themeProp;
      }
    ...
    const allColorSchemes = React15.useMemo(() => joinedColorSchemes.split(","), [joinedColorSchemes]);
    ...
    const memoTheme = React15.useMemo(() => {
      var _a2;
    ...
    const element = (0, import_jsx_runtime11.jsxs)(React15.Fragment, {
...
@mui/system/esm/index.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-3RZ35PZ7.js.map
